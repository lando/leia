...
assert.equal(process.env.LEIA_TEST_STAGE, stage === 'next' ? 'test' : stage);
assert.equal(process.env.LEIA_TEST_ID, 'lifecycle.scenario');
assert.equal(process.env.LEIA_TEST_NUMBER, stage === 'next' ? '2' : '1');
...
assert.equal(process.env.LEIA_PARSER_RETRY, '1');
...
const selected = mode.startsWith('setup-')
  ...
  : mode.startsWith('cleanup-')
    ...
    : 'test';
if (stage === selected) {
  if (mode.endsWith('failure')) process.exitCode = 7;
    ...
    fs.writeFileSync(`${trace}.ready`, 'ready');
    setInterval(() => {}, 1000);
