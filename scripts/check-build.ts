...
assert.deepEqual(Object.keys(first), ['cli.js', 'cli.js.map', 'index.js', 'index.js.map', 'package.json']);
await writeFile('dist/stale.js', 'stale output');
await run([process.execPath, 'run', 'build']);
assert.deepEqual(await snapshot(), first, 'Builds must remove stale files and emit identical bytes');
...
for (const flag of ['--help', '--version']) {
  ...
  const stableOutput = (value: string): string => flag === '--version' ?
    value.replace(/ node-v\d+\.\d+\.\d+\s*$/, '') : value;
  assert.equal(stableOutput(source), stableOutput(built), `Source and Node-built CLI disagree on ${flag}`);
...
try {
  ...
  await waitFor(async() => (await readFile('dist/index.js', 'utf8').catch(() => '')).includes(marker));
} finally {
  watcher.kill();
