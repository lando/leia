...
module.exports = (moduleFormat = 'auto', cwd = process.cwd()) => {
  if (!MODULE_FORMATS.includes(moduleFormat)) {
    throw new Error(
      `Unsupported module format "${moduleFormat}". Expected one of: ${MODULE_FORMATS.join(', ')}.`,
    );
  }
  ...
  while (true) {
    ...
    try {
      ...
      if (error.code !== 'ENOENT') {
        throw new Error(`Could not read nearest package.json at ${packageFile}: ${error.message}`, {
          cause: error,
        });
      }
    }
    ...
    if (source !== undefined) {
      try {
        ...
        throw new Error(
          `Could not parse nearest package.json at ${packageFile}: ${error.message}`,
          { cause: error },
        );
      }
    }
