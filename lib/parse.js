...
const parseCodeBlock = ({text, file}) => {
  ...
  return _.map(text.split(`${newLine}${newLine}`), (test) => ({
    ...
    script: path.posix.normalize(path.join(os.tmpdir(), 'leia', hash(file), `${hash(test)}.sh`)),
...
const parseTitle = (data, {retry, stdin}) => ({
  ...
  deps: path.posix.normalize(path.join(__dirname, '..', 'node_modules')),
  destination: path.resolve(os.tmpdir(), 'leia', hash(data.file), `${_.kebabCase(data.text)}.leia.js`),
  retry: retry,
  cwd: path.posix.normalize(path.dirname(path.resolve(data.file))),
