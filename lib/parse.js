 ...
 */
const validateTestHeader = (text, starters) =>
  !_.isEmpty(_.filter(starters, (begins) => _.startsWith(text, begins)));
...
const parseCodeBlock = ({ text, file, shell }) => {
  ...
  return _.map(text.split(`${newLine}${newLine}`), (test) => {
    ...
    const scriptPath = path
      .join(scriptDir, `${hash(test)}${shell.extension}`)
      .split(path.sep)
      .join('/');
    return {
      ...
      script: scriptPath,
      describe: _.map(
        _.filter(test.split(newLine), (line) => _.startsWith(line, '#')),
        parseTestDescribe,
      ),
    };
  });
 ...
 */
const parseTitle = (data, { moduleFormat, retry, stdin }) => ({
  file: data.file,
  ...
  destination: path.resolve(
    ...
    'leia',
    hash(data.file),
    `${_.kebabCase(data.text)}.leia.${moduleFormat === 'esm' ? 'mjs' : 'cjs'}`,
  ),
  moduleFormat,
  retry,
  ...
  _(data)
    ...
    .tap((groups) => {
      _.forEach(groups, (group, key) => {
        groups[key] = _.flatMap(group, (section) => section.tests).map((test, index) =>
          Object.assign(test, {
            id: group[0].id,
            number: index + 1,
            section: key,
    ...
    _(files)
      ...
      .map((datum) => {
        if (datum.type === 'heading' && datum.depth === 1) {
          const title = parseTitle(datum, {
            moduleFormat: resolvedModuleFormat,
            retry: resolvedRetry,
            stdin,
          });
      ...
      .map((datum) => {
        if (datum.type === 'heading' && datum.depth === 2) {
          ...
          debug(
            'found %o testing section candidate "%o" in %o',
            section.test,
            section.text,
            section.file,
          );
