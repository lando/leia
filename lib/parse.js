...
const parseCodeBlock = (text) => {
  ...
  return _.map(text.split(`${newLine}${newLine}`), (test) => {
    const parsedCommand = parseTestCommand(test);
    const argvedCommand = parseArgsStringToArgv(parsedCommand);
    const stringedArgs = _(_.tail(argvedCommand))
      .map((arg) => escapeString(arg))
      .value();
    return {
      argv: argvedCommand,
      command: parsedCommand,
      describe: _.map(_.filter(test.split(newLine), (line) => _.startsWith(line, '#')), parseTestDescribe),
      entrypoint: _.head(argvedCommand),
      args: _.tail(argvedCommand),
      argsString: stringedArgs.join(', '),
    };
  });
};
