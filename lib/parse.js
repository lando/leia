...
/*
 * Helper to remove unneeded headers from raw markdown data
 */
const trimHeaders = (datum, {testHeader, setupHeader, cleanupHeader}) => {
  if (datum.type === 'heading' && datum.depth === 1) return true;
  else if (datum.type === 'heading' && datum.depth < 3 && validateTestHeader(datum.text, setupHeader)) return true;
  else if (datum.type === 'heading' && datum.depth < 3 && validateTestHeader(datum.text, testHeader)) return true;
  else if (datum.type === 'heading' && datum.depth < 3 && validateTestHeader(datum.text, cleanupHeader)) return true;
...
module.exports = (files, {
  ...
  .filter(datum => trimHeaders(datum, {testHeader, setupHeader, cleanupHeader}))
  ...
  // Merge codeblock data into correct heading
  .value();
