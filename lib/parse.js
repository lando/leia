...
const parseCodeBlock = ({text, file}) => {
  ...
  return _.map(text.split(`${newLine}${newLine}`), (test) => {
    const scriptPath = path.join(os.tmpdir(), 'leia', hash(file), `${hash(test)}.sh`).split(path.sep).join('/');
    return {
      command: parseTestCommand(test),
      script: scriptPath,
      scriptArg: process.platform === 'win32' ? `"${scriptPath}"` : scriptPath,
...
const parseTitle = (data, {retry, stdin}) => ({
  ...
  deps: path.join(__dirname, '..', 'node_modules').split(path.sep).join('/'),
