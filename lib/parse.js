...
const parseCodeBlock = ({text, file}) => {
  ...
  return _.map(text.split(`${newLine}${newLine}`), (test) => ({
    ...
    script: path.relative(runDir, path.join(os.tmpdir(), 'leia', hash(file), `${hash(test)}.sh`)),
...
const parseTitle = (data, {retry, stdin}) => {
  const testDir = path.join(os.tmpdir(), 'leia', hash(data.file));
  return {
    file: data.file,
    id: _.kebabCase(data.text),
    deps: path.posix.relative(testDir, path.resolve(__dirname, '..', 'node_modules')),
    destination: path.join(testDir, `${_.kebabCase(data.text)}.leia.js`),
    retry: retry,
