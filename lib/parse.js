...
const getTestType = (datum, {testHeader, setupHeader, cleanupHeader}) => {
  if (validateTestHeader(datum.text, _.join(setupHeader, ','))) return 'setup';
  else if (validateTestHeader(datum.text, _.join(testHeader, ','))) return 'test';
  else if (validateTestHeader(datum.text, _.join(cleanupHeader, ','))) return 'cleanup';
  else return 'nope';
};
...
const parseTestCommand = test => {
  ...
  test = _(test.split('\n')).map(line => _.trim(line)).value().join('\n');
  // Regex to help assemble multiliner
  const mergeLines = new RegExp('\\\\\n', 'g');
  // String out comments
  const lines = _(test.replace(mergeLines, ' ').split('\n'))
 ...
 */
const parseCodeBlock = text => _.map(text.split('\n\n'), test => ({
  command: parseTestCommand(test),
  describe: _.map(_.filter(test.split('\n'), line => _.startsWith(line, '#')), parseTestDescribe),
}));
 ...
 */
const parseTitle = (data, {dest, outputExtension, retry, stdin}) => ({
  file: data.file,
...
module.exports = (files, {
  ...
  .map(datum => (datum.type === 'heading' && datum.depth === 1) ? parseTitle(datum, {dest, outputExtension, retry, stdin}) : datum)
