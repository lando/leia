...
const prepareHarness = (test) => {
  ...
  _.forEach(test.tests, (sectionTests, section) => {
    ...
    tests[section] = sectionTests.map((scenario, index) =>
...
const getRenderFunction = (template, opts = {}) => {
  ...
  return dot.template(
    ...
    _.merge({}, dot.templateSettings, opts),
    defs,
  );
};
...
module.exports = (tests, opts = { strip: false }) => {
  ...
  _.forEach(tests, (test) => {
    ...
    if (!templateFile)
      throw new Error(`Cannot generate unsupported module format "${moduleFormat}".`);
